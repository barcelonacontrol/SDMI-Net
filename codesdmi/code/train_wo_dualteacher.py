...
# 当前运行的方法
parser.add_argument('--method', type=str, default='XIAO_WO_DT', help='model_name')
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for i in range(T // 2):  # 取整除 等于4
                ...
                x[2 * i + 1: 2 * i + 2], opt[4 * i + 2: 4 * i + 4] = rot_flip.rot_num_flip(volume_batch_r[batch_size // 2 :], i, (i + 1) % 2)
            ...
            ema_output_arg = torch.argmax(ema_outputs, dim=1)
