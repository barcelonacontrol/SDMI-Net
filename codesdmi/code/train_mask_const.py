...
# 当前运行的方法
parser.add_argument('--method', type=str, default='XIAO_MASK_LINEAR', help='model_name')
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for i in range(T // 2):  # 取整除 等于4
                ...
                x[2 * i:2 * i + 1], opt[4 * i:4 * i + 2] = rot_flip.rot_num_flip(volume_batch_r[:batch_size // 2], i,
                                                                                 i % 2)
                x[2 * i + 1:2 * i + 2], opt[4 * i + 2:4 * i + 4] = rot_flip.rot_num_flip(
                    volume_batch_r[batch_size // 2:batch_size], i, (1 + i) % 2)
                ema_inputs = torch.cat((x[2 * i], x[2 * i + 1]), dim=0)

                ema_inputs1 = ema_inputs + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2,
                                                       0.2)  # 无标签数据 每次都加入噪声
                ema_inputs2 = ema_inputs + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2,
                                                       0.2)  # 无标签数据 每次都加入噪声

                with torch.no_grad():
                    ema_out1 = ema_model(ema_inputs1)
                    ema_out2 = ema_model2(ema_inputs2)
                    ema_out = 0.5 * (ema_out1 + ema_out2)
            ...
            # threshold = 0.75 * np.log(2)  # 一个标量控制指示函数的阈值

            # threshold = (0.75 + 0.25 * ramps.linear_rampup(iter_num, max_iterations)) * np.log(2)  # 一个标量控制指示函数的阈值
            #
            ...
            loss = supervised_loss + consistency_weight * consistency_dist + 0.1 * L_rmi
