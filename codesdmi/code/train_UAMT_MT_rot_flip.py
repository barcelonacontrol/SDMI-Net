...
# 当前运行的方法
parser.add_argument('--method', type=str, default='UAMT_MT_flip', help='model_name')
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for i in range(T // 2):  # 取整除 等于4
                ...
                opt = []
                x[2 * i:2 * i + 1], opt[2 * i:2 * i + 1] = rot_flip.rot_num_flip(volume_batch_r[:batch_size // 2], i,
                                                                                 i % 2)
                x[2 * i + 1:2 * i + 2], opt[2 * i + 1:2 * i + 2] = rot_flip.rot_num_flip(
                    volume_batch_r[batch_size // 2:batch_size], i, (1 + i) % 2)
                ema_inputs = torch.cat((x[2 * i:2 * i + 1], x[2 * i + 1:2 * i + 2]), dim=0)
                ema_inputs = ema_inputs + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2,
                                                      0.2)  # 无标签数据 每次都加入噪声
                with torch.no_grad():
                    ema_out1 = ema_model(ema_inputs)
                    ema_out2 = ema_model2(ema_inputs)
                    ema_out = 0.5 * (ema_out1 + ema_out2)
                    outA = rot_flip.get_origin(ema_out[:batch_size // 2], opt[2 * i:2 * i + 1])
                    outB = rot_flip.get_origin(ema_out[batch_size // 2:batch_size], opt[2 * i + 1:2 * i + 2])
            ...
            if iter_num % 250 == 0:
                ...
                logging.info(
                    ...
                    (iter_num, metric[0], metric[1], metric[2], metric[3]))
                logging.info('nowbest Dice: %f   nowbestteacher Dice: %f' % bestDice, bestDicet)
