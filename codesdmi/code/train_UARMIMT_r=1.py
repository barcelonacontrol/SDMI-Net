...
# 当前运行的方法
parser.add_argument('--method', type=str, default='XIAO_R=5', help='model_name')
...
if __name__ == "__main__":
    ...
    rmi = RMILoss(num_classes=2, rmi_radius=5)
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for i in range(T // 2):  # 取整除 等于4
                ...
                ema_inputs1 = ema_inputs + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)  # 无标签数据 每次都加入噪声
                ema_inputs2 = ema_inputs + torch.clamp(torch.randn_like(volume_batch_r) * 0.1, -0.2, 0.2)  # 无标签数据 每次都加入噪声
            ...
            logging.info('iteration %d : loss : %f , lsup: %f , lcons: %f, lmi: %f,  loss_weight: %f, lr: %f' % (
            iter_num, loss.item(), supervised_loss.item(), consistency_dist.item(), L_rmi.item(), consistency_weight, lr_))
